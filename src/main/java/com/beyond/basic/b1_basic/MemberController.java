...
//Controller 어노테이션을 통해 스프링에 의해 객체가 생성되고, 관리되어 개발자가 직접 객체를 생성 할 필요가 없음
...
//    case1. 서버가 사용자에게 text데이터 return
    ...
    @GetMapping("")
//    data(text,json) 리턴할때는 responsebody 사용. 화면(html)을 리턴할때에는 responsebody생략
...
//    case3-1. 정적인 html return
    @GetMapping("/html")
//    ResponseBody가없고, return타입이 String인경우 스프링은 templates폴더 밑에 simple_html.html을 찾아서 리턴
//    타임리프 의존성이 필요.
    public String htmlReturn(){
        return "simple_html";
    }
//    case3-2. 서버에서 화면+데이터를 함께주는  동적인 화면
//    현재 이 방식은 ssr(서버사이드렌더링)방식. csr방식은 화면은 별도제공하고, 서버는 데이터만 제공..
...
//    get요청의 url의 데이터 추출 방식 : pathvariable, 쿼리파라미터
...
//        별도의 형 변환 없이도,원하는 자료형으로 형변환되어 매개변수로 주입.(매개변수의 변수명이 url의 변수명과 일치해야함)
        System.out.println(inputId);
        return "OK";
    }

//    case2. parameter방식을 통한 url에서의 데이터추출(주로,검색,정렬 요청등의 상황에서사용)
...
//    형식 : body부에 name=hongildong&email=hong@naver.com
    ...
    @ResponseBody
//    형식이 url의 파라미터 방식과 동일하므로, RequestParam 또는 데이터바인딩 가능
...
//    형식 : body부에 name=hongildong&email=hong@naver.com&profileImage=xxxx(binary)
...
//    형식 : [{"name":"hong","email":"hong1@naver.com"},{"name":"hong","email":"hong2@naver.com"},{"name":"hong","email":"hong3@naver.com"}]
...
//    데이터 형식 : {"name":"hong","email":"hong1@naver.com","scores":[{"subject":"math","point":100},{"subject":"eng","point":90},{"subject":"korean","point":100}]}
    @PostMapping("/json-nested")
    @ResponseBody
    public String jsonnNested(@RequestBody Student student){
...
//    결론은 multipart-formdata구조안에 json을 넣는 방식.
    ...
    @ResponseBody
//    json과 file을 함께처리해야할때는 일반적으로 RequestPart 사용
